...
public sealed class RoundTurnQueueBuilder
{
    ...
    public RoundTurnQueues BuildRoundTurnQueues(
        ...
        IReadOnlySet<TurnParticipantKey> actedParticipants)
    {
        ...
        List<TurnParticipant> currentRoundQueue = currentRoundOrder
            .Where(participant => HasNotActed(participant, actedParticipants))
            .ToList();
    ...
    private static IEnumerable<TurnParticipant> GetAliveBeasts(
        ...
        => battleState.BeastTeam
            ...
            .Where(beast => IsBeastReadyToAct(beast, turnQueueProjection))
            .Select(beast => BuildBeastParticipant(beast, turnQueueProjection));

    private static bool HasNotActed(
        TurnParticipant participant,
        IReadOnlySet<TurnParticipantKey> actedParticipants)
        => !actedParticipants.Contains(new TurnParticipantKey(participant.Side, participant.BoardSlotIndex));
    ...
    private static bool IsBeastReadyToAct(
        BeastCombatUnit beast,
        TurnQueueProjection turnQueueProjection)
    {
